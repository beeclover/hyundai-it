...
interface SearchBarProps extends DOMAttributes<HTMLDivElement> {
  onChange?: (value: string) => void;
  items: string[];
  initialInputValue?: string;
}
const SearchBar = ({ onChange, items, initialInputValue, ...props }: SearchBarProps) => {
  return (
    <Downshift
      onChange={onChange}
            ...
            >
              <ClassNames>
                {({ css }) => (
                  ...
                  >
                    <ul tw="absolute z-10 mt-1 max-h-60 w-full overflow-auto bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                      {items
                        .filter(
                          item =>
                            !inputValue ||
                        ...
                        .map((item, index) => (
                          <li
                            css={[
                              tw`relative cursor-pointer select-none py-2 pl-8 pr-4`,
